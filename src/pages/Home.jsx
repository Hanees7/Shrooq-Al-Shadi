        ...
        {images.map((image, index) => (
          <div key={index} className="relative">
            <img
              src={image}
              alt={`Slide ${index + 1}`}
              className="w-full h-[70vh] sm:h-[60vh] lg:h-screen object-cover"

            />
            {index === 0 && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center bg-opacity-100 mt-70">
                <h2 className="text-white text-3xl md:text-5xl font-extrabold font-poppins whitespace-normal break-words text-center">
                  {["Granite Marble World - ", "Premium Stone Supply"].map((word, index) => (
                ...
                <p
                  className="text-white text-sm sm:text-base md:text-lg lg:text-lg font-semibold mt-6 font-poppins opacity-0 animate-softZoom max-w-3xl mx-auto px-4"
                  data-aos="zoom-in"
                  data-aos-delay="400"
                  data-aos-duration="1000"
                >
                  We deliver premium marble, granite, and natural stone for residential and commercial projects,
                  focusing on quality, durability, and design to enhance your space.
                </p>
              </div>
            )}
          </div>
